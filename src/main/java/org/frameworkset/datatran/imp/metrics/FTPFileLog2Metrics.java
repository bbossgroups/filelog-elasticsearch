...
/**
 * <p>Description:  从ftp服务器下载excel文件，采集excel文件中数据并交给指标计算器进行统计计算
 * 单个metrics，每个metrics只处理1类指标
 * </p>
 * <p></p>
 ...
 * @version 1.0
 */
public class FTPFileLog2Metrics {
	private static Logger logger = LoggerFactory.getLogger(FTPFileLog2Metrics.class);
	public static void main(String[] args){
		...
		FtpConfig ftpConfig = new FtpConfig().setFtpIP("localhost").setFtpPort(5322)
				.setFtpUser("1111").setFtpPassword("123456").setDownloadWorkThreads(4)
		...
		ETLMetrics keyMetrics = new ETLMetrics(Metrics.MetricsType_KeyTimeMetircs){
			...
			public void persistent(Collection< KeyMetric> metrics) {
				metrics.forEach(keyMetric->{
					...
					bulkProcessor.insertData("vops-keytimemetrics",esData);
		...
		importBuilder.setLastValueStorePath("FTPFileLog2Metrics_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
