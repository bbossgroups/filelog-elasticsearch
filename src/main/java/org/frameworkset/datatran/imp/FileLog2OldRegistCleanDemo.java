 ...
 * @version 1.0
 */
public class FileLog2OldRegistCleanDemo {
	private static Logger logger = LoggerFactory.getLogger(FileLog2OldRegistCleanDemo.class);
...
//				.setExcludeLines(new String[]{"\\[DEBUG\\]"}));//不采集debug日志
		...
		fileInputConfig.addConfig(new FileConfig("D:\\data\\filelivetime",//指定目录
				 ...
				 */
				.setEnableInode(false).setCloseOlderTime(10*1000L) //文件静默时间，超过这个时间文件内容没有变化，则将文件设置为已采集完毕，并关闭文件采集通道
...
//		importBuilder.setDummyOupputConfig(dummyOupputConfig);
		...
		importBuilder.setLastValueStorePath("filelogregistold_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
