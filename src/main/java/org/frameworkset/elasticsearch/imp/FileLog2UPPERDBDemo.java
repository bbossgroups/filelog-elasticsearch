 ...
 * @version 1.0
 */
public class FileLog2UPPERDBDemo {
	private static Logger logger = LoggerFactory.getLogger(FileLog2UPPERDBDemo.class);
...
//				.setExcludeLines(new String[]{"\\[DEBUG\\]"}));//不采集debug日志
		...
		config.addConfig(new FileConfig().setSourcePath("D:\\workspace\\bbossesdemo\\filelog-elasticsearch\\data")//指定目录
						...
						.setFileFilter(new FileFilter() {
							...
							public boolean accept(FilterFileInfo fileInfo, FileConfig fileConfig) {
								...
								return fileInfo.getFileName().equals("collection_module_20210926121012.json");
							}
						})//指定文件过滤器
						.setCloseEOF(false)//已经结束的文件内容采集完毕后关闭文件对应的采集通道，后续不再监听对应文件的内容变化
...
//				.setTargetUsePool(true)//是否使用连接池
				.setInsertSqlName("insertCOLLECTION_MODULE")//指定新增的sql语句名称，在配置文件中配置：sql-dbtran.xml
		...
		importBuilder.setLastValueStorePath("filelogdbuper_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
