 ...
 * @version 1.0
 */
public class FileLog2DBbatchDemo {
    private static Logger logger = LoggerFactory.getLogger(FileLog2DBbatchDemo.class);
    ...
    public static void main(String[] args) {
        ...
        //启动数据采集
        FileLog2DBbatchDemo file2DB = new FileLog2DBbatchDemo();
...
//		config.addConfig("E:\\ELK\\data\\data3",".*.txt","^[0-9]{4}-[0-9]{2}-[0-9]{2}");
        ...
        //String dbName = CommonLauncher.getProperty("db.name", "test");//获取目标数据库
        dbOutputConfig
                .setSqlFilepath("sql-dbtran.xml")
                //.setDbName(dbName)//指定目标数据库，在application.properties文件中配置
                //.setDeleteSql("deleteSql")
                //.setInsertSqlName(FileFilterPreName + "insertSql")//指定新增的sql语句名称，在配置文件中配置：sql-dbtran.xml
                //.setDeleteSql()
                //.setUpdateSql()
        ...
        //可以自行将采集的数据按照自己的要求进行处理到目的地，支持数据来源包括：database，elasticsearch，kafka，mongodb，hbase，file，ftp等，想把采集的数据保存到什么地方，有自己实现CustomOutPut接口处理即可
        //自己处理数据
        /*importBuilder.setCustomOutPut(new CustomOutPut() {
            ...
            public void handleData(TaskContext taskContext, List<CommonRecord> datas) {
                ...
                for (CommonRecord record : datas) {
                    ...
                    logger.info("自定义数据：" + SimpleStringUtil.object2json(data));
                }
            }
        });*/
        ...
        importBuilder.addCallInterceptor(new CallInterceptor() {
            ...
            public void preCall(TaskContext taskContext) {
                ...
                /*1、预处理:文件名处理*/
                FileTaskContext fileTaskContext = (FileTaskContext) taskContext;//文件任务上下文
                FileInfo fileInfo = fileTaskContext.getFileInfo();//文件信息
                String fileName = fileInfo.getFile().getName();//文件名
                taskContext.addTaskData("fileName", fileName);//任务中传递文件名
                ...
                String dstDbName = CommonLauncher.getProperty("db.name", "test"); //获取数据库名

                DBOutputConfig _dbOutputConfig = new DBOutputConfig();
                _dbOutputConfig.setDbName(dstDbName);//指定目标数据库，在application.properties文件中配置
                ...
                for (int i = 0; i < preFilesNameArr.length; i++) {
                    ...
                    if (nameMatch) {
                        //System.out.println("测试name:" + name);
                        //指定新增的sql语句名称，在配置文件中配置：sql-dbtran.xml
                        _dbOutputConfig.setInsertSqlName(preFilesNameArr[i] + "insertSql");
                        break;
                    }

                }

                taskContext.setDbmportConfig(_dbOutputConfig);//重构目标数据库配置
                ...
                logger.info("文件<" + fileName + ">预处理结束,耗时：" + (end - start) + " millis");
            }
...
//				Map fileMata = (Map)context.getValue("@filemeta");
        ...
        importBuilder.setAsyn(dataAsyn);//true 异步方式执行，不等待所有导入作业任务结束，方法快速返回；false（默认值） 同步方式执行，等待所有导入作业任务结束，所有作业结束后方法才返回
