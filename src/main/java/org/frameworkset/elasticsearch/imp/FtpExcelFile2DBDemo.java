 ...
 * @version 1.0
 */
public class FtpExcelFile2DBDemo {
	private static Logger logger = LoggerFactory.getLogger(FtpExcelFile2DBDemo.class);
...
//				.setUsePool(true)//是否使用连接池
		...
		importBuilder.setLastValueStorePath("ftpexcelfilelogdb_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
		//增量配置结束
