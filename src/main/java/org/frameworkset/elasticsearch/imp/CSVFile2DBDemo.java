 ...
 * @version 1.0
 */
public class CSVFile2DBDemo {
	private static Logger logger = LoggerFactory.getLogger(CSVFile2DBDemo.class);
...
//				.setUsePool(true)//是否使用连接池
		...
		importBuilder.setDataRefactor(new DataRefactor() {
			public void refactor(Context context) throws Exception  {
				...
				context.addFieldValue("person_no",values[1]);
				context.addFieldValue("name",values[2]);
				context.addFieldValue("cert_type",values[3]);
				context.addFieldValue("cert_no",values[4].toUpperCase());
